    ...
    */
    ...
    public function verifyid(Request $request, User $user)
    {
        if ($request->method() == "GET") 
        {
            ...
            $verifydata = verify_proof::where("user_id", "=", Auth::user()->id)->get()->first();
    ...
    public function history(Request $request, $name)
    {
        if ($request->method()  == "GET") {
            ...
            $data = Transaction::where("type", "=", config("app.transaction_type")[$type])
            ->where("user_id", "=", $user->id)->orderBy("created_at", "desc")->orderBy("status", "asc")->limit(30)->get();

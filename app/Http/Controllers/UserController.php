...
class UserController extends Controller
{
    ...
    public function verify(Request $request, $name, $id = null){
        ...
        if ($name == "approve") {
            ...
            $user = user::where("id", "=", $verifydata->user_id)->get()->first();
            // dd($userdata->email);
        ...
        if ($name == "decline") {
            ...
            try {
                Mail::to($user->email)->send(new GeneralMailer($details));
                Mail::to(config("app.admin_email"))->send(new GeneralMailer($admindetails5));
            } catch (\Exception $e) {
                // Never reached
            }
